...
static TreeNode * dowhile_stmt(void);//dowhile静态成员函数
static TreeNode * for_stmt(void);
static TreeNode * logic_exp(void);  //增加逻辑表达式
static TreeNode * logic_trem(void);
...
TreeNode * logic_exp(void)
{
    ...
    while(token == OR)
    {
        ...
        if(p!=NULL)
        {
            ...
            t = p;
            match(OR);
            t->child[1] = logic_trem();
        }
    }
    return t;
}

TreeNode * logic_trem(void)
{
